...
router.get('/list', checkAuth, userPermission, async (req, res) => {
    ...
    const {rows} = await db.query(`SELECT
                                    ...
                                    FROM tasks t
                                    WHERE t.reporter_employee_id = $1 AND project_id IN (SELECT project_id FROM project_members pm1 WHERE pm1.employee_id = $1 AND pm1.status = 1) AND deleted_at IS NULL ${filters.length > 0 ? `AND ${filters.join(' AND ')}` : ''}`, [req.currentUserId, ...values]);
                    ...
                    LEFT JOIN (
                        ...
                        FROM
                            task_activities
                                ...
                                WHERE (t.reporter_employee_id = $1 OR t.assigned_employee_id = $1) AND deleted_at IS NULL AND
                                    EXISTS (
                                        SELECT 1
