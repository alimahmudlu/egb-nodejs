...
router.get('/rating', checkAuth, async (req, res) => {
    const {rows: userDataRows} =
        await db.query(`
                SELECT
                    (SELECT sum(t.points), sum(t.finalPoints) FROM tasks t WHERE (assigned_employee_id = e.id OR reporter_employee_id = e.id) AND EXISTS (
                SELECT 1
                FROM task_activities ta
                WHERE ta.task_id = t.id
                  AND ta.status_id = 7
    ...
    const {rows: userDataRows} =
        await db.query(`
            SELECT
                ...
                exit.is_manual          AS exit_manual,
                entry.type              AS entry_type,
                exit.type               AS exit_type,
